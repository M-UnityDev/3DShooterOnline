...
class InterpolatedZombie:
    def __init__(self, entity):
        ...
        self.zactor = Actor('zombie/zombiea.gltf')
        self.zactor.reparent_to(entity)
        self.zactor.loop("walk")
...
def create_zombie():
    zombie = Entity(
        collider='box',
        collision_shape='box',
        collision_scale=(0.75, 0.75, 0.75),
    )
    return zombie
...
def create_other_player(id):
    ...
    # Создаем модель как Entity
    player_model = Entity(
        parent=parent,
        model='player/player.gltf',
        position=(0, -0.65, 0),
        collider='box',
        collision_shape='box',
        collision_scale=(1, 1, 1)
    )
    return parent
...
def initialize_game():
    ...
    try:
        ...
        client_socket.connect((IP, cport))
        ...
        # Загружаем модель зомби
        zombie_model = load_model('assets/models/zombie.obj')
        # Можно предзагрузить текстуру
        zombie_texture = load_texture('assets/textures/zombie_texture.png')
...
def update():
    ...
    if current_network_data is not None and 'players' in current_network_data:
        ...
        for pid, pdata in players_data.items():
            if pid != str(player_id):
                ...
                if pid in interpolated_players and pid in other_players:
                    new_pos = Vec3(pdata['x'], pdata['y'] + 2, pdata['z'])
                    old_pos = other_players[pid].position
                    interpolated_players[pid].set_target(new_pos)
                    
                    # Применяем поворот из данных сервера к модели
                    rotation = pdata.get('rotation', 0)
                    other_players[pid].rotation_y = rotation
                    
                    if not pdata['is_alive']:
                        other_players[pid].color = color.rgba(0, 0, 1, 0.5)
                    else:
                        other_players[pid].color = color.white
        ...
        if 'zombies' in current_network_data:
            ...
            for zid, zdata in zombies_data.items():
                if zdata['is_alive']:
                    ...
                    zombies[zid].scale = Vec3(0.75, 0.75, 0.75) * zdata.get('scale', 1)
    ...
    # Отправляем данные на сервер с поворотом камеры
    if is_alive:
        animation_state = 'idle'
        if held_keys['w'] or held_keys['a'] or held_keys['s'] or held_keys['d']:
            animation_state = 'run'
        
        if not hasattr(player, 'prev_y'):
            player.prev_y = player.y
        
        if player.y > player.prev_y + 0.1:
        ...
        send_data({
            ...
            'rotation': camera.rotation_y,  # Используем поворот камеры вместо игрока
